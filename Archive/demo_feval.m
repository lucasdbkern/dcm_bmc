...
% Convert cell arrays to 3D matrices for statistics
full_matrix = cat(3, full_activity{:});
ora_matrix = cat(3, ora_activity{:});
tra_matrix = cat(3, tra_activity{:});

% Calculate statistics across subjects (3rd dimension)
full_mean = mean(full_matrix, 3);
full_std = std(full_matrix, 0, 3);
ora_mean = mean(ora_matrix, 3);
ora_std = std(ora_matrix, 0, 3);
tra_mean = mean(tra_matrix, 3);
tra_std = std(tra_matrix, 0, 3);

% For plotting, assuming first row/channel of the activity matrix
time = time(:)';
