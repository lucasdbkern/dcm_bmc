function bmc2(file_paths, dcm_mat_file, customA_files, parameter_hE, parameter_hC, k)
% BMC 
...
for i = 1:n
    [Ms{i}.M.pE, Ms{i}.M.pC] = spm_dcm_neural_priors(Ms{i}.A, Ms{i}.B, Ms{i}.C, 'ERP'); %previously 'CMC'
...
if success 
   ...
   % results = struct('Ep', Ep, 'Cp', Cp, 'Eh', Eh, 'F', F, 'L', L, 'dFdp', dFdp, 'dFdpp', dFdpp);
    ...
    % Use the generated filename in the save command
    save(filename, 'F');
    %save('FreeEnergyMatrix_%d.mat', 'F', i);
  ...
  %  filename = ['BMC_Results_', custom_identifier, '.mat'];
   % save(filename, 'results');
    ...
    colorbar;
     ...
     disp('Free Energy (F) Matrix:')
    %filename = sprintf('FreeEnergyMatrix_%d_success.mat', i);
